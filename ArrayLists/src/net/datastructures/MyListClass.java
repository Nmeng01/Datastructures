...
public class MyListClass<E> implements List<E> {
	  ...
	  public MyListClass() { this(CAPACITY); }   // constructs list with default capacity

	  /** Creates an array list with given initial capacity. */
	  @SuppressWarnings({"unchecked"})
	  public MyListClass(int capacity) {         // constructs list with given capacity
	  ...
	  public MyListClass<E> switchTopBottom(MyListClass<E> list) {
		  int last = list.size() - 1;
		  int first = 0;
		  E temp1 = list.get(first);
		  E temp2 = list.get(last);
		  list.set(last, temp1);
		  list.set(first, temp2);
		  return list;
	  }
	  
	  public MyListClass<E> swap(int first, int last, MyListClass<E> list) {
		  E val1 = list.get(first);
		  E val2 = list.get(last);
		  list.set(first, val2);
		  list.set(last, val1);
